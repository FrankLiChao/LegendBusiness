...
-(void)showPick:(NSString*)currentValue data:(NSArray*)data valueChange:(CustomPickValueChange)changeBlock select:(CustomPickSelect)selectBlock disSelect:(CustomPickDisSelect)disSelect{
    ...
    self.selectValue = currentValue?currentValue:data.firstObject;
    self.oldValue = currentValue;;
...
-(void)showProvicePick:(NSString*)currentValue valueChange:(CustomPickValueChange)changeBlock select:(CustomPickSelect)selectBlock disSelect:(CustomPickDisSelect)disSelect;{
    ...
    RecieveAddressModel *model = [FileManagerSever getAddressModelWithDistrctID:currentValue];
    
    self.pickDataArray = [FileManagerSever getProvince];
    if (!model) {
        model = [RecieveAddressModel new];
        model.area_id = currentValue;
        model.provice = [_pickDataArray firstObject];
    }
    self.cityArray = [FileManagerSever getCityWithProvice:model.provice];
    if (!model.city ) {
        model.city = [_cityArray firstObject];
    }
    
    self.distrctArray = [FileManagerSever getDistrctWithProvice:model.provice city:model.city];
    if (!model.distrct) {
...
-(void)setUI{
    ...
    contentView = [[UIView alloc] initWithFrame:CGRectMake(0,
                                                           ...
                                                           [Configure SYS_UI_SCALE:180]+[Configure SYS_UI_BUTTON_HEIGHT])];
    contentView.backgroundColor = [UIColor whiteColor];
    [self addSubview:contentView];
    
    if (currentType == CustomPickType_Date) {
        datePicker = [[UIDatePicker alloc] initWithFrame:CGRectMake(0,0, [UIScreen mainScreen].bounds.size.width, 160)];
        ...
        pickView = [[UIPickerView alloc] initWithFrame:CGRectMake(0,0, [UIScreen mainScreen].bounds.size.width, 160)];
    ...
    button.backgroundColor = [Configure SYS_UI_COLOR_BG_RED];
    [button setTitle:@"确定" forState:UIControlStateNormal];
    [button setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    button.titleLabel.font = [UIFont systemFontOfSize:[Configure SYS_FONT_SCALE:16]];
    
    button.frame = CGRectMake(0,
                              contentView.frame.size.height- [Configure SYS_UI_BUTTON_HEIGHT],
                              [UIScreen mainScreen].bounds.size.width,
                              [Configure SYS_UI_BUTTON_HEIGHT]);
...
-(void)selectClick:(UIButton*)button{
    ...
    if (self.selectBlock) {
        ...
        if (currentType == CustomPickType_Date) {
            ...
            NSString *zipCode = [FileManagerSever getDistrctIDWithModel:self.selectAddresssModel];
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {//选择后操作
    ...
    if (currentType == CustomPickType_SingleRow) {
        ...
        if (component == PROVINCE_COMPONENT) {
            ...
            self.cityArray = [FileManagerSever getCityWithProvice:proviceStr];
            NSString *cityStr = [_cityArray firstObject];
            self.distrctArray = [FileManagerSever getDistrctWithProvice:proviceStr city:cityStr];
            ...
            self.distrctArray = [FileManagerSever getDistrctWithProvice:self.selectAddresssModel.provice city:cityStr];
