...
-(void)initUI{
    ...
    [closeButton setImage:[UIImage imageNamed:@"use_icon_31.png"] forState:UIControlStateNormal];
    [closeButton addTarget:self action:@selector(clickClose) forControlEvents:UIControlEventTouchUpInside];
    [contentView addSubview:closeButton];
    
    UILabel *titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(60, 10, contentView.frame.size.width - 120, 30)];
    ...
    for (int i = 0; i<6; i++) {
        UIImageView *imageV = [[UIImageView alloc] initWithFrame:CGRectMake(15 + i*(spaceW + perW), contentView.frame.size.height - perW - 20, perW, perW)];
        imageV.tag = i+1;
        imageV.contentMode = UIViewContentModeCenter;
        [contentView addSubview:imageV];
        [UitlCommon setFlatWithView:imageV radius:5 borderColor:speateLine.backgroundColor borderWidth:1];
    }
