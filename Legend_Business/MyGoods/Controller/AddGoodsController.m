...
- (IBAction)clikSaveButton:(id)sender{
    ...
    if (self.specArray.count>0) {
        ...
        for (AddGoodsCell4 *cell in self.specArray) {
            ...
            NSDictionary *dic = [NSDictionary dictionaryWithObjectsAndKeys:cell.priceField.text,@"price",cell.stockField.text,@"goods_number",cell.specsField.text,@"attr_name", nil];
    ...
    for (CustomUploadImageView *view in  self.currentGoodPicViews) {
    ...
    [self addGoods:cell1.titleField.text categoryId:self.selectGoodChildGategory.cat_id price:price goodsNum:stock shareMoney:[cell6.contentField.text floatValue] goodsBrief:_goodsDes goodsPicDes:goodsPics galleryImage:@[cell1.coverImage.url] goodsImage:cell1.homePageImage.url goodsThumb:self.goodsthumb attrList:spesArray isPrepare:bOpen_pre_sale prepareTime:[cell7.contentField.text floatValue]*24*60*60 sellerTip:_sellerNotice isEndorse:NO shippingFee:@"" shippingFree:@"" success:^(NSString *goodId) {
        [weakSelf hideHud];
        
        UIAlertView *alter = [[UIAlertView alloc] initWithTitle:@"恭喜您"
                                                        message:@"添加商品成功，是否还要继续添加?"
                                                       delegate:weakSelf
                                              cancelButtonTitle:@"不"
